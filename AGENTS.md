...
- **网易云速记浮窗（P1a，✅ 已实现并经真机验证）**: `Ctrl+Alt+S` 唤起**按需面板**（用完 **destroy** 不是 hide，不常驻 / 不轮询 / 不拖拽）→ 读网易云主窗口标题 → 匿名搜索反查真实 id → 可打标签、写批注、插入**估算**时间戳（UI 带「约」字，因为窗口标题里没有播放进度）。反查不到时**允许凭空记**：用合成 id `np:{曲名}|{歌手}` 落库（`unresolved: true`）。批注走 `notes::save_notes`，Obsidian 同步自动生效。设置页可开关 + 有「打开面板」兜底按钮（快捷键被占用时用户看不到注册失败的告警，只有 eprintln）。
...
- **统计页（数据可视化）**: 侧栏新增「统计」入口（`AppView` 加 `stats`），后端 `get_collection_stats(range_days)` 聚合（总数 / 星标数 / 未打标签数 / 按来源 / 标签 Top10 / 按月时间线，均 `deleted_at IS NULL` 软删过滤；`range_days>0` 时全部聚合限定 `favorite_time >= now-range*86400`，供时间筛选）；前端 `StatsPage` 用手写 SVG 环形图 + 水平条 + 竖向时间线，**不引入图表库**。仅基于本地元数据，切到该页才拉取。
- **统计页下钻**：头部「全部 / 近30天 / 近90天」分段控件重拉 `get_collection_stats(range_days)`；环形扇区 + 图例 + 标签 Top10 整行**可点击**，经 `App.libraryDrill`（`{source}` / 用标签名在 `tags` 解析 `{tagId}`）跳收藏库并应用一次筛选（`LibraryPage` 新增 `drill` prop，应用后回调清空，支持相同来源重复点击）。
- **跨源重复项检测与合并**：侧栏新增「重复项」入口（带组数 badge，启动即算）。后端 `find_duplicate_groups` 在 Rust 侧分组——**零 schema 改动、不改任何插入路径**（规避 `items` 表 10+ 处 INSERT 的漏列风险）。两层匹配：① `normalize_url` host 感知精确分组（去 `www.` / 忽略 scheme / 网易云 `#/song?id=` 还原 + `?id=` 保留 / 其余丢跟踪参数；空 URL 跳过避免假重复），`match_type="url"`；② **模糊层**：未被 url 组覆盖、标题非空的 item 按**字符 2-gram 重叠系数（containment）**聚类（`title_similarity`，阈值 `FUZZY_THRESHOLD=0.8`，对「主体相同、末/首追加转存者或画质标注」的中文重复鲁棒），`match_type="fuzzy"`，UI 虚线+问号标「疑似」、只提示不自动合并。`merge_duplicate_items` 把待删项标签并集（`INSERT OR IGNORE`）+ **批注重合并**（`merge_notes`：保留项优先，待删非空且不同者用 `---` 分隔追加、去重）写保留项后**软删进回收站**（同一事务，可恢复），末尾重建保留项 FTS 并同步 Obsidian（`notes::save_notes`，失败不阻塞）。前端 `DuplicatesPage` 每组选保留项 + 合并其余；`DuplicatesListener`（ToastProvider 内）启动/导入/合并后**仅组数增加时** toast（localStorage 记录，避免每次打开打扰）。
